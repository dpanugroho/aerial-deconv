...
#    ksize = config.ksize
    ...
    encoder6 = tf.nn.conv2d(encoder4, _W['ce6'], strides=[1, 1, 1, 1], padding='SAME')
    if use_bias:
    ...
    encoder6 = tf.nn.max_pool(encoder6, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
    encoder6 = tf.nn.dropout(encoder6, _keepprob)
    print("Encoder 6", encoder6)
    
    # 8x8
    # Decoder 8x8 (128/16 = 8) fsize: 64
    decoder6 = Unpooling(encoder6, [tf.shape(_X)[0], height / 64, width / 64, fsize])
    decoder6 = tf.nn.conv2d_transpose(decoder6, _W['cd6']
    ...
    decoder6 = tf.nn.dropout(decoder6, _keepprob)
    print("Decoder 6", decoder6)
    
    
    decoder5 = Unpooling(encoder5, [tf.shape(_X)[0], height/32, width/32, fsize])
    decoder5 = tf.nn.conv2d(decoder5, _W['cd5'], strides=[1, 1, 1, 1], padding='SAME')
